...
public class DownloadController {
    ...
    public String download(Model model){
        ...
        model.addAttribute("files",listFilesUsingJavaIO(uploadedDirectory.getAbsolutePath()));
        return "list_files";
    }
    ...
    public ResponseEntity<?> download(@RequestParam("file") String fileName, HttpServletResponse response){
        ...
        byte[] fileBytes=null;
        try {
            fileBytes= Files.readAllBytes(Paths.get(uploadedDirectory.getAbsolutePath()+
                    "/"+
                    fileName));
        } catch (IOException e) {
